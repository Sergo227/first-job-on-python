...
def toChange_data(filename):
    ...
    change_tel_book_lines = tel_book_lines[index_delete_data]
    elements = change_tel_book_lines.split(" | ")
    fio = input("Введите ФИО: ")
    ...
    if len(phone) == 0:
        phone = elements[2]
    change_line = f"{num} | {fio} | {phone}"
    tel_book_lines[index_delete_data] = change_line
    print(f"Запись - {change_tel_book_lines}, изменена на - {change_line}\n")
...
def main():
    ...
    while my_choice != 0:
        ...
        if action == 1:
            ...
            record_data(file_tel)
        elif action == 3:
            toChange_data(file_tel)
        elif action == 4:
